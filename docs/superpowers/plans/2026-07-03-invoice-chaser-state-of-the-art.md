...
1. **Single Next.js monolith** on Vercel; background work via Inngest (already integrated). No microservices — YAGNI at this scale; services layer keeps future extraction possible.
...
- [ ] **Vercel** project linked to the GitHub repo; preview deployments on PRs; production env. — deferred, USER ACTION (no Vercel CLI/link in this environment); instructions in `docs/setup/PROVISIONING.md`. Owner: user.
- [x] **Postgres** production DB (Supabase or Neon) — **decision: Supabase**, existing project "Invoice Chaser" (`sikdvtqrdqynknlvpsls`) adopted, see `docs/setup/PROVISIONING.md`. Preview/branch DB choice + `prisma migrate` cutover still deferred to Phase 1/pending Vercel linkage. Owner: user (preview DB decision).
...
1. Vercel production: env audit, custom domain, security headers/CSP, Clerk prod keys.
